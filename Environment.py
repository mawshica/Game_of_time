...
class Environment:
    ...
    def __init__(self, map_x, map_y, name, time_machine):
        ...
        if(time_machine):
            ...
            self.setState(machine_x, machine_y, True)
    ...
    def printMap(self):
        for y in range(self.Map[0].__len__()):
            ...
            for x in range(self.Map.__len__()):
                if (self.getState(x,y)):

                    print(" ", end='')
                    print(self.Map[x][y].getSymbol(), end='')
                    print(" ", end='')
                else:
                    print(" . ", end='')
            print()
    ...
    def setState(self, x_pos, y_pos, alive):
        if(x_pos < 0 or x_pos >= self.Map.__len__()):
            return False
...
class Node:
    ...
    def step(self, env):
        ...
        if(env.getState(self.x_pos + 1, self.y_pos)):
            neighbors_alive += 1;

        if(env.getState(self.x_pos - 1, self.y_pos)):
            neighbors_alive += 1;

        if (env.getState(self.x_pos, self.y_pos + 1)):
            neighbors_alive += 1;

        if (env.getState(self.x_pos, self.y_pos - 1)):
            neighbors_alive += 1

        if (env.getState(self.x_pos + 1, self.y_pos + 1)):
            neighbors_alive += 1
...
class TimeComputerNode(Node):
    ...
    def step(self, env):
        ...
        if(self.alive and (neighbors_alive >= 4)):
            if(self.compute_state(env.initial, env.generation + 20, ("Copy of " + env.name))):
                env.setState(self.x_pos, self.y_pos - 1, False)
                env.setState(self.x_pos + 1, self.y_pos - 1, False)
                env.setState(self.x_pos - 1, self.y_pos - 1, False)
                env.setState(self.x_pos + 1, self.y_pos, False)
                env.setState(self.x_pos - 1, self.y_pos, False)
                env.setState(self.x_pos, self.y_pos + 1, False)
                env.setState(self.x_pos + 1, self.y_pos + 1, False)
                env.setState(self.x_pos - 1, self.y_pos + 1, False)
